...
public static class FloatExpressionBuilder
{
    ...
    private static Expression BuildComparisonExpression(Expression property, string filterValue, Operator filterOperator)
    {
        ...
        return filterOperator switch
        {
            ...
            _ => throw new InvalidOperationException($"Invalid operator {filterOperator} for float.")
        };
    }
    ...
    private static Expression BuildInExpression(Expression property, string filterValue)
    {
        ...
        {
            if (!float.TryParse(value.Trim(), NumberStyles.Any, CultureInfo.InvariantCulture, out float floatValue))
                throw new FormatException($"Invalid float format: {value}");
            return Expression.Equal(property, Expression.Convert(Expression.Constant(floatValue), property.Type));
        }).ToArray();
    ...
    private static Expression BuildBetweenExpression(Expression property, string filterValue)
    {
        ...
        if (!float.TryParse(values[0].Trim(), NumberStyles.Any, CultureInfo.InvariantCulture, out float minValue) || 
            !float.TryParse(values[1].Trim(), NumberStyles.Any, CultureInfo.InvariantCulture, out float maxValue))
            throw new FormatException("Invalid float format in between values");
